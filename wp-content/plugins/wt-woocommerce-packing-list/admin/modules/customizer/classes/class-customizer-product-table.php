...
trait Wf_Woocommerce_Packing_List_Customizer_Product_table
{
	...
	*/
	public static function sort_items($items, $template_type, $order, $doc_type="non-package", $sort_by="name", $sort_order="asc")
	{
		$out=array();
										...
										foreach ($epo_tc_meta_data as $key => $epo) 
										{
											if ($epo && is_array($epo)) 
											{
												...
												$tmcart_option_qty = $epo['quantity'];
											 	 ...
											 	 $meta_data_formated_arr[] ='<small style="line-height:18px;">'.$tmcart_option_name .':'.' <span style="white-space: pre-wrap;">'. $tmcart_option_value.'</span><br>'. ' COST: ' . $tmcart_option_price .'<br>Qty: '. $tmcart_option_qty .'</small><br>';
											}
										}
									}
						    	}
