...
class Wf_Woocommerce_Packing_List_Creditnote
{
	...
	*/
	...
	public function set_design_view_data($find_replace, $html, $template_type)
	{
		...
		$find_replace['[wfte_creditnote_date]'] = date('d/M/Y');
		return $find_replace;
	}
	...
	{	
		...
		$credit_date = self::get_creditnote_date($order_id,'d/M/Y',$order);
		//echo "creditdate".$credit_date;
	...
	*/
	public function alter_quantity_column($qty, $template_type, $_product, $order_item, $order)
	{
		...
		if($new_qty<0)
		{
			$qty='<span style="">'.absint($new_qty).'</span>';
		}else{
			$qty = "-";
		}
		return $qty;
	...
	*/
	public function alter_total_price_column($product_total_formated, $template_type, $product_total, $_product, $order_item, $order)
	{
		...
		if($new_total > 0){
			$product_total_formated = '<span style="">'.$product_total_formated.'</span>';
		}else{
	...
	*/
	...
	public function alter_total_tax_column($item_tax_formated,$template_type,$item_tax,$_product,$order_item,$order){
		...
		foreach($tax_items as $tax_item){
    		...
    		$tax_amount=(float)$tax_item->amount;
    		$refund_tax += (float)$order->get_tax_refunded_for_item($item_id,$tax_rate_id);
		}
	...
	*/
	public function alter_shipping_row($shipping, $template_type, $order, $product_table){
		$wc_version=WC()->version;
		$refunded_shipping = abs((float)$order->get_total_shipping_refunded());
		$refund_amount=0;
		$refund_data_arr=$order->get_refunds();
		if(!empty($refund_data_arr))
		{
			...
			{	
				...
				$cr_refund_amount=abs((float) get_post_meta($refund_id,'_order_shipping_tax',true));
				$refunded_shipping+=$cr_refund_amount;
			}
		}
	...
	*/  
	public function alter_extra_tax_row($tax_amount, $tax_item, $order, $template_type,$tax_rate_id){
		...
		foreach($order->get_items() as $item_id => $order_item){
			$refund_tax += $order->get_tax_refunded_for_item($item_id,$tax_rate_id);
		}
		$tax_amount = Wf_Woocommerce_Packing_List_Admin::wf_display_price($user_currency,$order,$refund_tax);
