...
class THWCFE_Public extends WCFE_Checkout_Fields_Utils{
		...
		if( isset( $file['name'] ) && is_array( $file['name'] ) ){
			...
			for( $i=0; $i < $count; $i++ ){
				...
				if(is_array($file_new) && isset($file_new['name'])){
					...
					$file_new['name'] = apply_filters('thwcfe_uploaded_file_name', $file_new['name'], $file_new, $fname);
	...
	****************************************/
    ...
    public function woo_form_field($ofield = '', $key, $args, $value = null ) {
        ...
        $field_container = '<p class="form-row %1$s" id="%2$s" data-priority="' . esc_attr( $sort ) . '" data-rules="'.$rules.'" data-rules-action="'.$rules_action.'" data-validations="'.$validations_str.'">%3$s</p>';
