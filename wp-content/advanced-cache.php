...
$possible_plugin_locations = array(
	...
	'/Users/jj3/Projects/ritchies/wp-content/plugins/wp-optimize-premium/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://ritchiessports.test/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/Users/jj3/Projects/ritchies/wp-content/plugins/wp-optimize-premium/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-ritchiessports.test.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-ritchiessports.test.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-ritchiessports.test.php');
}
